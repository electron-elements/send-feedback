...
class SendFeedback extends HTMLElement {
  constructor() {
    ...
    // Add the attribute here if you want the value of it
    // to be updated in the element when either it's property
    // or attribute is changed. Note: the property with hypens are
    // converted to camel case. For example, title-placeholder attribute,
    // is this.titlePlaceholder.
  ...
  connectedCallback() {
    ...
    // Make sure to add code for the removing the event listener
    // in the disconnectedCallback if you add a event listener.
    ...
    this._cancel.addEventListener('click', this._feedbackCancelledHandler.bind(this));
    ...
    if (this.showCancelButton == 'show' || this.showCancelButton == 'true') {
  ...
  disconnectedCallback() {
    ...
    this._cancel.removeEventListener('click', this.feedbackCancelledEvent.bind(this));
  }

  _feedbackCancelledHandler() {
  ...
  hideLoader(error = false) {
    ...
    setTimeout(() => {
      ...
      if (showCancelButton == 'show' || showCancelButton == 'true') {
